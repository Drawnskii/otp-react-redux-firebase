import { ACCOUNT_PATH, PERSIST_TO_FIREBASE } from './constants'
...
/**
 * Obtains the Firebase configuration if the following applies in config.yml:
 * - persistence.strategy is 'firebase' (and persistence is enabled),
 * - persistence.firebase is defined.
 * @param persistence The OTP persistence configuration from config.yml.
 * @returns The Firebase configuration, or null if the conditions are not met.
 */
export function getFirebaseConfig(persistence) {
  if (!persistence || persistence.strategy !== PERSIST_TO_FIREBASE) return null
  return persistence.firebase || null
}

/**
 * Checks if Firebase authentication is enabled
 * @param persistence The OTP persistence configuration from config.yml.
 * @returns Boolean indicating if Firebase auth is enabled
 */
export function isFirebaseEnabled(persistence) {
  return !!getFirebaseConfig(persistence)
}
