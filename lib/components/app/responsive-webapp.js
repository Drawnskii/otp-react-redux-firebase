...
import { getAuth0Config, getFirebaseConfig, isFirebaseEnabled } from '../../util/auth'
...
import FirebaseAuthProvider from '../user/firebase-auth-provider'
...
class RouterWrapperWithAuth0 extends Component {
  ...
  render() {
              ...
              <QueryParamProvider adapter={ReactRouter5Adapter}>
                <Switch>
                  {routes.map((props, index) => {
                    ...
                    // Use ProtectedRoute for routes that need authentication
                    if (protectedRoute) {
                      return (
                        <ProtectedRoute
                          key={index}
                    ...
                    // For routes with getContextComponent or shouldRenderWebApp, use standard Route
                    return (
                      <Route
                        component={
                          getContextComponent
                  ...
                  {/* For any other route, simply return the web app via a protected route */}
                  <ProtectedRoute render={() => <WebappWithRouter {...this.props} />} />
                </Switch>
              </QueryParamProvider>
            </ConnectedRouter>
          </IntlProvider>
    ...
    // Wrap the router with the appropriate auth provider
...
const mapStateToWrapperProps = (state) => {
  ...
  return {
    ...
    firebaseConfig: getFirebaseConfig(persistence),
    homeTimezone,
    locale: state.otp.ui.locale,
    localizedMessages: state.otp.ui.localizedMessages,
    routerConfig: reactRouter,
    useFirebase: isFirebaseEnabled(persistence)
  }
}
